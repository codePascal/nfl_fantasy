...
for position in POSITIONS:
    ...
    for _, row in df_pos.iterrows():
        plt.plot((row["BEST"], row["WORST"]), (row["RK"], row["RK"]), c=colors.get(row["TIERS"], "black"), alpha=0.8)
    ...
    plt.title(f"{position} Tiers for 2022 Draft - ECR vs Average Expert Rank", fontsize=12)

    plt.gca().invert_yaxis()
    plt.tight_layout()
    plt.savefig("../plots/draft_ranking/tier_ranking_{}.png".format(position))
